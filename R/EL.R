                    ...
                    ...)
{
    ...
    p <- ggplot2::ggplot(df, ggplot2::aes(x = t, y = estim)) +
        ggplot2::geom_line(color = "blue") +  # Main line
        ggplot2::labs(x = if ("xlab" %in% names(optional))
            optional$xlab
            else
                dxlab,
            y = if ("ylab" %in% names(optional))
                optional$ylab
            else
                eval(dylab)) +
    ...
    if (!is.null(conf.level)) {
        p <- p +
            ...
            ggplot2::geom_line(ggplot2::aes(y = lower_conf),
                               linetype = "dashed",
                               color = "red") +  # Lower bound
            ggplot2::geom_line(ggplot2::aes(y = upper_conf),
                               linetype = "dashed",
                               color = "red")   # Upper bound
    }
                      ...
                      more.warnings = FALSE)
{
    ...
    mm <- function(globals, tt)
    {
        ...
        conffun <- function(conf.level)
        {
            function(k)
                withCallingHandlers(
                    ELconf(globals, X, Y, tt[k], conf.level, delta = d[k]),
    ...
    if (warnings)
        warning(
            "Estimates or confidence bands for some values could not be found. For a detailed report, run this command again with 'more.warnings = TRUE'.",
            call. = FALSE
        )
    res
}
...
{
    tr <- c(min(X), max(X))
    c(tr[1] - max(Y) + globals$precision,
      tr[2] - min(Y) - globals$precision)
}
...
deltarange.roc <- function(globals, X, Y, t)
{
    ...
    c(min(range) + globals$precision,
      max(range) - globals$precision)
}
